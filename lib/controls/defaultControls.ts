...
export const createCornerControl = (position, cursorAngle, actionHandler) => {
  return new fabric.Control({
    x: position.x,
    y: position.y,
    offsetX: 0,
...
export const defaultControls = (rect) => {
  return {
    ml: createSideControl(
      ...
      45,
      fabric.controlsUtils.scalingX // Use the Fabric.js scaling handler
    ),
